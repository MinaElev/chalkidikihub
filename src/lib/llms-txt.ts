...
import { getVillagesList, getBeaches, type VillageRow } from '@/lib/data';
...
The site is published in 7 languages: Greek (default, unprefixed), English (/en/), German (/de/), Bulgarian (/bg/), Russian (/ru/), Romanian (/ro/), and Serbian (/sr/). All pages listed below exist in each locale — replace the prefix as needed.
...
- Per-entity markdown: \`/api/md/{listings|beaches|restaurants|places}/{slug}\` (returns clean Markdown for LLM ingestion; supports \`?locale=\`)
- Per-locale variants of this file: \`/{en,de,bg,ru,ro,sr}/llms.txt\`
`;
...
export async function generateLlmsTxt(locale: string): Promise<string> {
  const [villages, beaches] = await Promise.all([
