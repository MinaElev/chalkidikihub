...
export default function AdminEditVillagePage() {
  const { id } = useParams();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const supabase = createClient();
    const { error: err } = await supabase.from('villages').update({
      ...form,
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Village</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
        <AIHelper titleEl={form.name_el as string} descriptionEl={form.description_el as string}
          ...
          onComplete={(data) => {
            setForm(prev => ({
              ...
              name_bg: data.translations.title_bg, name_ru: data.translations.title_ru, name_ro: data.translations.title_ro, name_sr: data.translations.title_sr || "",
        ...
        <div className="flex items-center gap-3 p-3 bg-purple-50 border border-purple-200 rounded-xl">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-purple-600">Γράφει <strong>περιγραφή EL</strong> (HTML formatted) + εκτιμά <strong>πληθυσμό</strong> βάσει ονόματος χωριού.</p>
            <p className="text-[10px] text-purple-400 mt-0.5">⚡ Μετά πατήστε AI Auto-Complete (Βήμα 2) για μεταφράσεις + SEO · Κόστος: ~$0.01</p>
          </div>
