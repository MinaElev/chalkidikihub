...
export default function NewActivityPage() {
  ...
  const tCategory = useTranslations('activityCategories');
  ...
  const [form, setForm] = useState<Record<string, unknown>>({
    ...
    category: 'historical', price_range: '', duration: '',
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
        ...
        <AIHelper
          ...
          onComplete={(data) => {
            setForm((prev) => ({
              ...
              name_ru: data.translations.title_ru, name_ro: data.translations.title_ro, name_sr: data.translations.title_sr || "",
              ...
              description_ru: data.translations.description_ru, description_ro: data.translations.description_ro, description_sr: data.translations.description_sr || "",
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Image</label>
            <ImageUpload currentUrl={form.image_url as string} onUpload={(url) => update('image_url', url)} folder="activities" /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Category</label>
            <select value={form.category as string} onChange={(e) => update('category', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-primary-500">
              {ALL_ACTIVITY_CATEGORIES.map((c) => <option key={c} value={c}>{tCategory(c)}</option>)}
            </select></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Price Range</label>
            <input type="text" value={form.price_range as string} onChange={(e) => update('price_range', e.target.value)}
              placeholder="e.g. 10-30"
              className="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-primary-500" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Duration</label>
            <input type="text" value={form.duration as string} onChange={(e) => update('duration', e.target.value)}
