// Push sales description rewrites to Supabase sales table.
const fs = require('fs');
...
(async () => {
  const data = JSON.parse(fs.readFileSync(path.join(__dirname, 'beach-rewrites', 'sales.json'), 'utf8'));
  ...
  for (const [slug, payload] of Object.entries(data)) {
    ...
    const res = await fetch(`${SUPABASE_URL}/rest/v1/sales?slug=eq.${encodeURIComponent(slug)}`, {
      method: 'PATCH',
  ...
  if (!DRY && changed.length) {
    console.log(`\nRevalidating ${changed.length} sale(s) on the live site...`);
    await revalidate('sales', changed);
  }
})();
